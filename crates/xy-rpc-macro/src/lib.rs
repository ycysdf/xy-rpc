...
pub fn rpc_service(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let (rpc_call_fn, rpc_call_fn_impl, handle_impl, wasm_rpc_call_proxy): (
        Vec<_>,
        Vec<_>,
        Vec<_>,
        Vec<_>,
        ...
        .map(|(i, trait_item_fn)| {
            ...
            for (i, n) in wasm_rpc_call_fn.inputs.iter_mut().enumerate() {
                ...
                if segment.ident == "TransStream" {
                    ...
                    let param_type_ident = format_ident!(
                        "{}{}{i}ParamTy",
                        ast.ident,
                        trait_item_fn.sig.ident.to_string().to_case(Case::UpperCamel)
                    );
            ...
            wasm_rpc_call_fn.output = match wasm_rpc_call_fn.output {
                ...
                ReturnType::Type(_, ty) => {
                    ...
                    if let Some(item_ty) = &output_stream_item {
                        let return_type_ident = format_ident!(
                            "{}{}{i}ReturnStreamItemTy",
                            ast.ident,
                            trait_item_fn.sig.ident.to_string().to_case(Case::UpperCamel)
                        );
                        wasm_extra.push(quote! {
                        ...
                        let return_type =
                            format!("AsyncIterable<{}[0]>", return_type_ident);
            ...
            let item_ident =
                format_ident!("{}", trait_item_fn.sig.ident.to_string().to_case(Case::UpperCamel));
            ...
            let rpc_type = match (stream_arg, &output_stream_item) {
            ...
            (
                ...
                quote! {
                    #rpc_call_fn {
                        let future = self.call(&#ref_ident { #fields }, #msg_stream, #id as _, #rpc_type);
                        async move {
                                ...
                                pub #wasm_rpc_call_fn {
                                    ...
                                    let r = self.rpc_channel.#fn_ident(#(&#fields),* #d futures_util::stream::unfold(js_stream, move |mut state| async move {
                                        ...
                                        match r {
                                            ...
                                            Err(err) => {
                                                Some((Err(xy_rpc::RpcError::OtherError {
                                                    message: format!("{err:?}"),
                                                }), state))
                                            }
                                        }
    ...
    let impls = quote! {
            ...
            T: #trait_ident,
        {
            fn handle(
                &self,
                msg: xy_rpc::RpcRawMsg,
                stream: Option<xy_rpc::flume::Receiver<xy_rpc::bytes::Bytes>>,
                serde_format: &impl xy_rpc::formats::SerdeFormat,
            ) -> Result<impl core::future::Future<Output = Result<xy_rpc::HandleReply, xy_rpc::RpcError>> + xy_rpc::maybe_send::MaybeSend, xy_rpc::RpcError> {
        ...
        impl<CF> #caller_ident for xy_rpc::XyRpcChannel<CF, #schema_ident>
        where
            CF: xy_rpc::formats::SerdeFormat,
        {
            #(#rpc_call_fn_impl)*
        }
    };
