...
use zyn::syn::token::Comma;
use zyn::syn::{
    FnArg, GenericArgument, GenericParam, Generics, Ident, ItemTrait, Lifetime, LifetimeParam, Pat,
    PatType, PathArguments, PathSegment, ReturnType, TraitItem, TraitItemFn, Type, TypeParamBound,
    TypeReference, parse_quote,
};
use zyn::{Span, TokenStream, format_ident, zyn};

#[zyn::element]
fn msg_struct_decl<'a, 'b>(
    trait_fn_item: &'a TraitItemFn,
    trait_item: &'b ItemTrait,
) -> zyn::TokenStream {
    let found_input_trans_stream = get_input_trans_stream(trait_fn_item);
    let args = trait_fn_item
        .sig
        .inputs
        .iter()
        .enumerate()
        .filter(|n| found_input_trans_stream.as_ref().is_none_or(|s| s.3 != n.0))
    ...
    let ident = format_ident!(
        "{}{}",
        zyn::case::to_pascal(trait_item.ident.to_string().as_str()),
        zyn::case::to_pascal(trait_fn_item.sig.ident.to_string().as_str())
    );
    let ref_ident = format_ident!("{ident}Ref",);
    zyn! {
        #[derive(serde::Serialize, serde::Deserialize, Debug)]
        {{trait_item.vis}} struct {{ident}} {
        ...
        {{trait_item.vis}} struct {{ref_ident}} @if (!args.is_empty()) {<'a>} {
            @for (arg in args.iter()) {
                {{arg.pat}}: &'a {{arg.ty}},
            }
        }
...
#[zyn::attribute]
fn rpc_service(#[zyn(input)] item: ItemTrait) -> zyn::TokenStream {
    let mut item = item;
    let schema_ident = format_ident!("{}Schema", item.ident);
    let variant_ident = format_ident!("{}Variant", item.ident);
    let caller_ident = format_ident!("{}Caller", item.ident);
    ...
    let trait_fn_items = item
        .items
        .iter()
        .filter_map(|n| match n {
            TraitItem::Fn(n) => Some(n),
            _ => None,
        })
        .collect::<Vec<_>>();
    let tokens_variant = {
        ...
        zyn! {
            ...
            enum {{variant_ident}}<{{generics}}> {
                @for (i in 0..trait_fn_items.len()) {
                    {{trait_fn_items[i].sig.ident | pascal }}({{i| ident: "T{}"}}),
                }
            }
        }
    };
    ...
    let (tokens_impl_rpc_msg_handler, tokens_caller, wasm_export) = {
        let ref_lifetime = Lifetime::new("'a", Span::call_site());

        let (rpc_call_fn, rpc_call_fn_impl, handle_impl, wasm_rpc_call_proxy): (Vec<_>, Vec<_>, Vec<_>, Vec<_>) = trait_fn_items.iter()
            .enumerate()
            .map(|(i, trait_item_fn)| {
                let id = (i + 1) as u16;
                let stream_arg = get_input_trans_stream(trait_item_fn);
                let output_stream_item = get_output_stream_item(trait_item_fn);
                let fields: Punctuated<Ident, Comma> = trait_item_fn
                    .sig
                    .inputs
                    .iter()
                    .enumerate()
                    .filter(|n| stream_arg.is_none_or(|a| a.3 != n.0))
                ...
                for (i, n) in wasm_rpc_call_fn.inputs.iter_mut().enumerate() {
                    ...
                    if segment.ident == "TransStream" {
                        ...
                        let param_type_ident = format_ident!("{}{}{i}ParamTy",item.ident,zyn::case::to_pascal(trait_item_fn.sig.ident.to_string().as_str()));
                        wasm_extra.push(zyn! {
                            #[cfg(target_arch = "wasm32")]
                            #[wasm_bindgen::prelude::wasm_bindgen(getter_with_clone)]
                            pub struct {{param_type_ident}}(pub {{item_type}});
                        });
                        let param_type = format!("AsyncIterator<{}[0]>", param_type_ident);
                ...
                for (i, arg) in rpc_call_fn.inputs.iter_mut().enumerate() {
                    if let FnArg::Typed(arg) = arg {
                        if stream_arg.is_some_and(|n| n.3 == i) {
                            ...
                            *arg.ty = Type::Reference(TypeReference {
                ...
                wasm_rpc_call_fn.output = match wasm_rpc_call_fn.output {
                    ...
                    ReturnType::Type(_, ty) => {
                        ...
                        if let Some(item_ty) = &output_stream_item {
                            let return_type_ident = format_ident!("{}{}{i}ReturnStreamItemTy",item.ident,zyn::case::to_pascal(trait_item_fn.sig.ident.to_string().as_str()));
                            wasm_extra.push(zyn! {
                                #[cfg(target_arch = "wasm32")]
                                #[wasm_bindgen::prelude::wasm_bindgen(getter_with_clone)]
                                pub struct {{return_type_ident}}(pub {{item_ty}});
                            });
                            let return_type = format!("AsyncIterable<{}[0]>", return_type_ident);
                            wasm_attr = Some(zyn! {
                ...
                let item_ident = format_ident!("{}",zyn::case::to_pascal(trait_item_fn.sig.ident.to_string().as_str()));
                let ident = format_ident!("{}{item_ident}",item.ident);
                let ref_ident = format_ident!("{}{item_ident}Ref",item.ident);
                ...
                let rcp_type = match (stream_arg, &output_stream_item) {
                    (None, None) => zyn!(xy_rpc::Unary),
                    (None, Some(..)) => zyn!(xy_rpc::ReplyStreaming),
                    (Some(..), None) => zyn!(xy_rpc::MsgStreaming),
                    (Some(..), Some(..)) => zyn!(xy_rpc::BidirectionalStreaming)
                };
                (
                    zyn! {
                        {{rpc_call_fn}}
                    ...
                    zyn! {
                        {{rpc_call_fn}} {
                            let future = self.call(&{{ref_ident}} {
                                @for(n in &fields) {{{n}},}
                            }, {{msg_stream}}, {{id}} as _, {{rcp_type}});
                            async move {
                        ...
                        match stream_arg {
                            None => zyn! {
                                {{id}} => {
                                    ...
                                    let {{ident}} {
                                        @for (n in &fields) {{{n}},}
                                    } = serde_format.deserialize_from_slice_optimized(&msg.msg).map_err(xy_rpc::RpcError::SerdeError)?;
                            ...
                            None => {
                                ...
                                zyn! {
                                    ...
                                    pub {{wasm_rpc_call_fn}} {
                                        let r = self.rpc_channel.{{fn_ident}}(
                                            @for(n in fields) {
                                                {{n}},
                                            }
                                ...
                                zyn! {
                                    ...
                                    pub {{wasm_rpc_call_fn}} {
                                      ...
                                      let r = self.rpc_channel.#fn_ident(
                                            ...
                                            futures_util::stream::unfold(js_stream,move |mut state| async move {
        ...
        (
            ...
            zyn! {
                {{item.vis}} trait {{caller_ident}} {
                    @for (n in rpc_call_fn) {
                        {{n}};
                    }
                }
                impl<CF> {{caller_ident}} for xy_rpc::XyRpcChannel<CF,{{schema_ident}}> where CF: xy_rpc::formats::SerdeFormat {
                    @for (n in rpc_call_fn_impl) {
                        {{n}}
                    }
                }
            },
            {
                let wasm_channel_proxy = format_ident!("{}ChannelProxy", item.ident);
                let channel_fn_ident = zyn! { {{item.ident| snake}}};
                zyn! {
                    #[cfg(target_arch = "wasm32")]
                    #[wasm_bindgen::prelude::wasm_bindgen]
                    pub async fn {{channel_fn_ident}}(url: &str) -> Result<{{wasm_channel_proxy}}, wasm_bindgen::JsValue> {
    ...
    zyn::zyn!(
        ...
        {{item.vis}} struct {{schema_ident}};
        impl xy_rpc::RpcSchema for {{schema_ident}} {}

        {{tokens_variant}}

        @for (n in trait_fn_items.iter()) {
            @msg_struct_decl(
                trait_fn_item = &n,
                trait_item = &item
            )
